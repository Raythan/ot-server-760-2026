...
function onSay(player, words, param)
    ...
    if(param == '') or (param == 'info') then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Commands to use this system:\n!offer balance\nUse this command to check the balance of your sales.\n!offer add, itemname, price, qty\nexample: !offer add,plate armor,500,1\n\n\n!offer buy,purchase_id\nexample: !offer buy,1943\n\n\n!offer remove, purchase_id\nexample: !offer remove, 1943\n\n\n!offer withdraw, quantity\nexample: ! offer withdraw, 1000.\n")
        return true
    end
    ...
    if(split[1] == "add") then
        ...
        if(string.len(split[3]) > 7 or (string.len(split[4]) > 3)) then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "This price or quantity items is too high.")
            return true
        end
        local itemType, item_s = ItemType(split[2]), ItemType(split[2]):getId()
        if(not item_s) then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "The item "..itemType.." does not exist!")
            return true
        end
        ...
        if(player:getItemCount(item_s) < tonumber(split[4])) then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, you do not have this item.")
            return true
        end
        ...
        if _BUSCA_DB ~= false then
            ...
            if limit then
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, but you have reached the maximum limit(" .. config.max_ofertas .. ") of publications selling items.")
                return true
            end
            if(config.SendOffersOnlyInPZ) then    
            if(not getTilePzInfo(player:getPosition())) then
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, but you can only use this command while in the protection zone.")
                return true
            end
        end
    end
    ...
    player:sendTextMessage(MESSAGE_INFO_DESCR, "Congratulations! You have added for sale" .. itemcount .." " .. split[2] .." for " .. costgp .. " gps!")
end
if(split[1] == "buy") then
    ...
    if(_BUSCA_DB ~= false) then
        ...
        if(player:getMoney() < costs) then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, but you do not have the necessary amount to purchase. Are necessary: "..costs.."gps to buy the item: "..item_names..". You have: " .. player:getMoney() .. "gps. You need to: "..total_custo.." gps.")
            return true
        end
        if(player_id == player_vendas) then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, you cannot purchase your own item.")
        return true
    end
...
if(split[1] == "withdraw") then
    ...
    player:sendTextMessage(MESSAGE_INFO_DESCR, "You got it " .. split[2] .. " GPS of your sales in the market! Your balance is: "..tz.."gps.")
    player:addMoney(tz)
