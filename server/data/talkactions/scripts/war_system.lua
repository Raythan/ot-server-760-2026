-- !war invite, enemyGuild, frags(x), money(x) [0] // -- !war invite, enemyGuild, frags(x), money(x), timeDays(x) [0]
...
-- <talkaction words="!war" separator=" " script="war_system.lua" />
...
function onSay(player, words, param, channel)
	...
	if table.contains({"accept", "reject", "cancel"}, split[1]) then
		...
		if split[1] == "accept" then
			for _, guildPlayer in ipairs(guild:getMembersOnline()) do
				guildPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, "War started. You must relog in to upgrade shields and emblems.")
			end
	...
	elseif split[1] == "invite" then
		local invitationDenied, resultId = "", db.storeQuery("SELECT `guild1`, `status` FROM `guild_wars` WHERE `guild1` IN (" .. guildId .. "," .. enemyGuildId .. ") AND `guild2` IN (" .. enemyGuildId .. "," .. guildId .. ") AND `status` IN (0, 1)")
		if resultId then
		...
		if ending and ending ~= 0 then
			local timeDaysMax = warSystem.daysMaxToInviteWar
		...
		db.query("INSERT INTO `guild_wars` (`guild1`, `guild2`, `name1`, `name2`, `started`, `ended`, `frags`, `payment`) VALUES (" .. guildId .. ", " .. enemyGuildId .. ", " .. db.escapeString(guildName) .. ", " .. db.escapeString(enemyName) .. ", " .. begining .. ", " .. ending .. ", " .. fragLimit .. ", " .. payment .. ");")
