...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	player->sendIcons();
	// re-send opened containers
	for (const auto& it : player->openContainers) {
		const Container* container = it.second.container;
		sendContainer(it.first, container, container->hasParent(), player->getContainerIndex(it.second.index));
	}
}
...
void ProtocolGame::sendOutfitWindow()
{
	...
	for (const Outfit& outfit : outfits) {
		...
		/*if (protocolOutfits.size() == 15) { // Game client doesn't allow more than 100 outfits
			break;
		}*/
	}
