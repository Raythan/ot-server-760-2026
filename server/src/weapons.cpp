	...
	if (!player->hasFlag(PlayerFlag_IgnoreWeaponCheck)) {
		...
		if (auto chance = g_config.getNumber(ConfigManager::CRITICAL_HIT_CHANCE)) {
			if (boolean_random(static_cast<double>(chance) / 100.0)) {
				...
				g_game.addAnimatedText("Crit!", target->getPosition(), TEXTCOLOR_RED);
			}
		}
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	damage.value = -normal_random(0, maxDamage);
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if (it.hitChance == 0) {
		...
		const Position& playerPos = player->getPosition();
		const Position& targetPos = target->getPosition();
		uint32_t distance = std::max<uint32_t>(1, std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos), Position::getDistanceY(playerPos, targetPos)));
