...
void ProtocolSpectator::addSpectator(ProtocolGame_ptr spectator) {
    ...
    sendCastMessage("", spectator->getSpectatorName() + " has joined the cast", TALKTYPE_CHANNEL_O);
...
void ProtocolSpectator::removeSpectator(ProtocolGame_ptr spectator) {
    ...
    sendCastMessage("", spectator->getSpectatorName() + " has left the cast", TALKTYPE_CHANNEL_O);
    setUpdateStatus(true); // update spectators count
}
...
void ProtocolSpectator::spectatorSay(ProtocolGame_ptr spectator, const std::string& text) {
    if(spectator->isSpectatorMuted()) {
        sendCastMessage("", "You are temporary muted", TALKTYPE_CHANNEL_O, spectator);
        return;
    }
    ...
    sendCastMessage(spectator->getSpectatorName(), text, TALKTYPE_CHANNEL_Y);
}
...
void ProtocolSpectator::sendCastMessage(const std::string& author, const std::string& text, SpeakClasses type, ProtocolGame_ptr target /*=nullptr*/) {
    if(!target)
        return sendChannelMessage(author, text, type, 0xFFFE);

    target->sendChannelMessage(author, text, type, 0xFFFE);
}