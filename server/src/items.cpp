			...
			while (true) {
				while (true) {
					...
					if (identifier == "waypoints") {
						...
						items[id].hitChance = script.readNumber();
					} else if (identifier == "maxhitchance") {
						items[id].maxHitChance = script.readNumber();
					} else if (identifier == "walkstack") {
						items[id].walkStack = script.readNumber();
			...
			while (true) {
				while (true) {
					...
					if (identifier == "type") {
						...
						count = std::max<uint32_t>(1, script.readNumber());
					} else if (identifier == "start") {
						start = std::max<uint32_t>(0, script.readNumber());
					} else if (identifier == "damage") {
						damage = -script.readNumber();
						if (start > 0) {
