...
void BehaviourDatabase::checkAction(const NpcBehaviourAction* action, Player* player, const std::string& message)
{
	...
	case BEHAVIOUR_TYPE_BURNING: {
		const int32_t baseDamage = 10;
		int32_t rounds = evaluate(action->expression, player, message);
		int32_t damage = baseDamage + evaluate(action->expression2, player, message);
		...
		conditionDamage->addDamage(rounds, 5000, -damage);
	...
	case BEHAVIOUR_TYPE_POISON: {
		int32_t rounds = evaluate(action->expression, player, message);
		/*int32_t damage =*/ evaluate(action->expression2, player, message);
		...
		ConditionDamage::generateDamageList(rounds, 5, damageList);
		for (int32_t damageValue : damageList) {
			conditionDamage->addDamage(1, 5000, -damageValue);
		}
