...
private:
	...
	void sendPrivateMessage(const Player* speaker, SpeakClasses type, const std::string& text) {
		...
		for(auto& it : spectators)
			it->sendPrivateMessage(speaker, type, text);
	}
	...
	void sendAnimatedText(const Position& pos, uint8_t color, const std::string& text) {
		...
		for(auto& it : spectators)
			it->sendAnimatedText(pos, color, text);
	}
	...
	void sendRemoveRuleViolationReport(const std::string& name) {
		if(owner)
			owner->sendRemoveRuleViolationReport(name);
		
		for(auto& it : spectators)
			it->sendRemoveRuleViolationReport(name);
	}
	...
	void sendRuleViolationCancel(const std::string& name) {
		...
		for(auto& it : spectators)
			it->sendRuleViolationCancel(name);
	}
	...
	void sendRuleViolationsChannel(uint16_t channelId) {
		...
		for(auto& it : spectators)
			it->sendRuleViolationsChannel(channelId);
	}
	...
	void sendAddTileItem(const Position& pos, const Item* item, uint32_t stackpos) {
		if(owner)
			owner->sendAddTileItem(pos, item, stackpos);
		
		for(auto& it : spectators)
			it->sendAddTileItem(pos, item, stackpos);
	}
