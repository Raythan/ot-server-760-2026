...
void Monster::doAttacking(uint32_t interval)
{
	...
	for (const spellBlock_t& spellBlock : mType->info.attackSpells) {
		...
		if (canUseSpell(myPos, targetPos, spellBlock, interval, inRange, resetTicks)) {
			if (static_cast<uint32_t>(uniform_random(0, spellBlock.chance) == 0)) {
				if (updateLook) {
					updateLookDirection();
					updateLook = false;
				}
                          ...
                          const spellBlock_t& sb, uint32_t interval, bool& inRange, bool& resetTicks)
{
	...
	if (sb.isMelee && (OTSYS_TIME() - lastMeleeAttack) < 2000) {
		return false;
	}
			...
			if (canChangeTarget) {
				...
				if (targetChangeTicks >= mType->info.changeTargetSpeed) {
					...
					if (mType->info.changeTargetChance >= uniform_random(1, 100)) {
						...
						int32_t random = uniform_random(0, 100);
						TargetSearchType_t searchType = TARGETSEARCH_ANY;

						if (random <= mType->info.strategyNearestEnemy) {
							searchType = TARGETSEARCH_NEAREST;
						}
						
						if (random <= mType->info.strategyWeakestEnemy) {
							searchType = TARGETSEARCH_WEAKEST;
						}
						
						if (random <= mType->info.strategyMostDamageEnemy) {
							searchType = TARGETSEARCH_MOSTDAMAGE;
						}
						
						if (random <= mType->info.strategyRandomEnemy) {
...
void Monster::onThinkDefense(uint32_t interval)
{
	...
	for (const spellBlock_t& spellBlock : mType->info.defenseSpells) {
		...
		if (!uniform_random(0, spellBlock.chance)) {
			minCombatValue = spellBlock.minCombatValue;
			maxCombatValue = spellBlock.maxCombatValue;
...
bool Monster::getNextStep(Direction& direction, uint32_t& flags)
{
	...
	if ((!followCreature || !hasFollowPath) && (!isSummon() || !isMasterInRange)) {
		if (OTSYS_TIME() >= nextDanceStepRound) {
            nextDanceStepRound = OTSYS_TIME() + 850 + getStepDuration();
		...
		if (result) {
			...
			if (attackedCreature && attackedCreature == followCreature) {
				...
				} else if (targetDanceCooldown <= 0 && mType->info.staticAttackChance < static_cast<uint32_t>(uniform_random(1, 100))) {
					result = getDanceStep(getPosition(), direction);
					targetDanceCooldown += 1500 + getStepDuration();
				}
			}
		}
	}
