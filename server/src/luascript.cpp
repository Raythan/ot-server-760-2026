...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnumIn("configKeys", ConfigManager::RATE_REGENERATION)
	...
	registerMethod("Player", "getDepotChest", LuaScriptInterface::luaPlayerGetDepotLocker);
...
{
	// player:getDepotLocker(depotId[, autoCreate = false])
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
