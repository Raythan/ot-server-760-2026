...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		damage.value = normal_random(min, max);
			...
			if (params.valueCallback) {
				...
				damage.value = normal_random(
					...
					if (params.useCharges) {
						const ItemType& itemType = Item::items.getItemType(tool->getID());
						if (itemType.charges) {
							int32_t newCount = std::max<int32_t>(0, tool->getCharges() - 1);
							if (newCount <= 0) {
								g_game.internalRemoveItem(tool);
							} else {
								g_game.transformItem(tool, tool->getID(), newCount);
							}
						}
					}
				} else {
