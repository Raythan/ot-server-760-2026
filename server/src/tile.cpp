			...
			if (!monster->hasCondition(CONDITION_AGGRESSIVE)) {
				if (hasFlag(TILESTATE_FIREDAMAGE) && !monster->isImmune(COMBAT_FIREDAMAGE)) {
					if (hasBitSet(FLAG_IGNOREFIELDDAMAGE, flags)) {
						if (!(monster->canPushItems() || monster->hasCondition(Combat::DamageToConditionType(COMBAT_FIREDAMAGE)))) {
							return RETURNVALUE_NOTPOSSIBLE;
						}
					} else {
						return RETURNVALUE_NOTPOSSIBLE;
				...
				if (hasFlag(TILESTATE_POISONDAMAGE) && !monster->isImmune(COMBAT_EARTHDAMAGE)) {
					if (hasBitSet(FLAG_IGNOREFIELDDAMAGE, flags)) {
						if (!(monster->canPushItems() || monster->hasCondition(Combat::DamageToConditionType(COMBAT_EARTHDAMAGE)))) {
							return RETURNVALUE_NOTPOSSIBLE;
						}
					} else {
						return RETURNVALUE_NOTPOSSIBLE;
				...
				if (hasFlag(TILESTATE_ENERGYDAMAGE) && !monster->isImmune(COMBAT_ENERGYDAMAGE)) {
					if (hasBitSet(FLAG_IGNOREFIELDDAMAGE, flags)) {
						if (!(monster->canPushItems() || monster->hasCondition(Combat::DamageToConditionType(COMBAT_ENERGYDAMAGE)))) {
							return RETURNVALUE_NOTPOSSIBLE;
						}
					} else {
						return RETURNVALUE_NOTPOSSIBLE;
		...
		if (itemIsHangable && hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
			...
			if (items) {
				for (const Item* tileItem : *items) {
					...
					if (iiType.blockSolid && item->isBlocking()) {
						return RETURNVALUE_NOTENOUGHROOM;
					}
					if (!iiType.blockSolid) {
						continue;
...
void Tile::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
    ...
    const TileItemVector* items = getItemList();
    if (items) {
        // To determine if a letter/parcel can be sent, we need to count the number
        // of items on top of the mailbox to ensure there's nothing blocking it.
        for (auto it = items->getBeginDownItem(), end = items->getEndDownItem(); it != end; ++it) {
            topItemCount++;
        }
    }

    // The letter/parcel is already on the tile (on top of the mailbox),
    // so we need to check that only one item was counted. Then we can be
    // sure that it's not being blocked by another item. Note that the client
    // doesn't know the letter/parcel is on the tile yet, just the engine knows.
    if (topItemCount == 1) {
