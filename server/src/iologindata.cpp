...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if(mount == nullptr || !player->hasMount(mount)){
		player->defaultOutfit.lookMount = 0;
	} else {
		player->toggleMount(result->getNumber<uint16_t>("ridingmount") == 1);
	}
	...
	//load opened containers
	std::set<uint16_t> saved_containers;
	query.str(std::string());
	query << "SELECT `index` FROM `player_containers` WHERE `player_id` = " << player->getGUID()<< " ORDER BY `index` DESC";
	if ((result = db->storeQuery(query.str()))) {
		do {
			int32_t index = result->getNumber<uint32_t>("index");
			saved_containers.insert(index);
		} while (result->next());
	}
	...
	uint16_t player_container_id = 0;
	std::list<Container*> queue;
	std::map<uint16_t, Container*> containers;
	...
	while (!queue.empty()) {
		Container* container = queue.front();
		queue.pop_front();
	...
	for (const auto& it : saved_containers) {
		const auto& container = containers.find(it);
		if (container == containers.end()) {
			continue;
		}
		player->addContainer(player_container_id++, container->second);
	}
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, SavedContainers* saved_containers)
{
	...
	while (!queue.empty()) {
		...
		if (saved_containers) {
			saved_containers->addContainer(container);
		}
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	query << "`ridingmount` = " << (player->isMounted() ? 1 : 0) << ',';
	...
	// uint8_t-based enum streams as a character, not a number — breaks SQL (e.g. 1 -> SOH)
	...
	if (!saveItems(player, itemList, itemsQuery, propWriteStream, &containers)) {
		return false;
	}
	...
	if (!saveItems(player, itemList, depotQuery, propWriteStream, nullptr)) {
		return false;
	}
...
if (!storageQuery.execute()) {
	...
	query << "DELETE FROM `player_containers` WHERE `player_id` = " << player->getGUID();
	if (!db->executeQuery(query.str())) {
		return false;
	}

	query.str(std::string());
	DBInsert containersQuery("INSERT INTO `player_containers`(`player_id`, `index`) VALUES ");

	for (const auto& it : player->openContainers) {
		if (const Container* container = it.second.container) {
			if (container->getHoldingPlayer() != player) {
				continue;
			}

			for (auto container_it = containers.containers.begin(); container_it != containers.containers.end();) {
				if (container_it->container == container) {
					// printf("Saving container (id:%d, name:%s)\n", container_it->index, container->getName());
					query << player->getGUID() << ',' << static_cast<int32_t>(container_it->index);
					container_it = containers.containers.erase(container_it);
					if (!containersQuery.addRow(query)) {
						return false;
					}
				} else {
					container_it++;
