...
bool Game::internalStartTrade(Player* player, Player* tradePartner, Item* tradeItem)
{
	...
	if (tradePartner->tradeState == TRADE_NONE) {
		...
		tradePartner->sendTextMessage(MESSAGE_INFO_DESCR, ss.str());
...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
	...
	if (result == TALKACTION_BREAK) {
		int32_t value;
        player->getStorageValue(30019, value);
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field)
{
	...
	static const auto sendBlockEffect = [this](BlockType_t blockType, CombatType_t combatType, const Position& targetPos) {
		if (blockType == BLOCK_DEFENSE) {
			...
			uint8_t hitEffect = 0;
			uint8_t hitEffectFire = 0;
			uint8_t hitEffectIce = 0;
			switch (combatType) {
				...
				case COMBAT_FIREDAMAGE: {
					hitEffect = CONST_ME_BLOCKHIT;
					hitEffectFire = CONST_ME_HITBYFIRE;
					addMagicEffect(targetPos, hitEffectFire);
					break;
				}
				
				case COMBAT_ICEDAMAGE: {
					hitEffect = CONST_ME_BLOCKHIT;
					hitEffectIce = CONST_ME_ICEATTACK;
					addMagicEffect(targetPos, hitEffectIce);
					break;
				}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.value > 0) {
		...
		if (Monster* monster = target->getMonster()) {
			// makes monsters aggressive state when damaged
			// only strong monsters are available to get aggressive
			// aggressive monsters walk over fields
			if (!monster->hasCondition(CONDITION_AGGRESSIVE)) {
				if (monster->canPushCreatures()) {
					Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_AGGRESSIVE, 1000);
					monster->addCondition(condition, true);
					monster->updateMapCache();
				}
			}
		}
